...
require_once __DIR__ . '/candy_agent.php';
...
class ManagePanel
{
    ...
    function createUser($name_panel, $code_product, $usernameC, array $Data_Config)
    {
        ...
        } elseif ($Get_Data_Panel['type'] == "candy_agent") {
            //create user
            $ConnectToPanel = create_user_candy($Get_Data_Panel, $data_limit / pow(1024, 3), ($expire - time()) / 86400, $usernameC);
    ...
    function DataUser($name_panel, $username)
    {
        ...
        if ($Get_Data_Panel['type'] == "marzban") {
            ...
            $UsernameData = get_user_data_candy($Get_Data_Panel, $username);
            if (!empty($UsernameData['error'])) {
                $Output = array(
                    'status' => 'Unsuccessful',
                    'msg' => $UsernameData['error']
    ...
    function Revoke_sub($name_panel, $username)
    {
        ...
        if ($Get_Data_Panel['type'] == "marzban") {
            ...
            $revoke_sub = revoke_service_candy($Get_Data_Panel, $username);
    ...
    function RemoveUser($name_panel, $username)
    {
        ...
        if ($Get_Data_Panel['type'] == "marzban") {
            ...
            $UsernameData = remove_service_candy($Get_Data_Panel, $username);
            if (isset($UsernameData['error'])) {
                $Output = array(
                    'status' => 'Unsuccessful',
                    'msg' => $UsernameData['msg']
    ...
    function extend($Method_extend, $new_limit, $time_day, $username, $code_product, $name_panel)
    {
        ...
        if ($panel['type'] == "marzban") {
            ...
            $extend = extend_service_candy($panel, $new_limit, $time_day, $username);
    ...
    function extra_volume($username_account, $code_panel, $limit_volume_new)
    {
        ...
        if ($panel['type'] == "marzban") {
            ...
            $volume_add = add_volume_service_candy($panel, $limit_volume_new, $username_account);
    ...
    function extra_time($username_account, $code_panel, $limit_time_new)
    {
        ...
        if ($panel['type'] == "marzban") {
            ...
            $time_add = add_time_service_candy($panel, $new_limit, $username_account);
