...
INSTALL_LOG="/tmp/candy_install.log"
...
ensure_dns() {
    ...
    elif [ -f "$RESOLV" ] && [ ! -f "${RESOLV}.candy.bak" ]; then
        cp -a "$RESOLV" "${RESOLV}.candy.bak" 2>/dev/null
    fi
...
# Ensure /usr/local/bin/candy points at the master script
_link_candy() {
...
function self_update_script() {
    ...
    local BIN_LINK="/usr/local/bin/candy"
    local URL="https://raw.githubusercontent.com/ednoct/candybot/main/install.sh"
    local TEMP_FILE="/tmp/candybot_update.sh"
    ...
    if [ "$valid" -ne 1 ]; then
        ...
        _link_candy "$MASTER_PATH" "$BIN_LINK"
        return 0
    fi
...
# ── Repo / paths ─────────────────────────────────────────────
BOT_DIR_DEFAULT="/var/www/html/candyprobotconfig"
CONFIG_FILE_DEFAULT="$BOT_DIR_DEFAULT/config.php"
GIT_REPO="ednoct/candybot"
LATEST_CACHE="/tmp/.candy_latest_version"
IP_CACHE="/tmp/.candy_server_ip"
...
STATE_DIR="/root/confcandy"
STATE_FILE="$STATE_DIR/.candy_install_state"
...
setup_mysql_root() {
    sudo mkdir -p /root/confcandy || return 1
    touch /root/confcandy/dbrootcandy.txt || return 1
    sudo chmod -R 777 /root/confcandy/dbrootcandy.txt || return 1
    ...
    echo "\$user = 'root';"               >> /root/confcandy/dbrootcandy.txt
    echo "\$pass = '${randomdbpasstxt}';" >> /root/confcandy/dbrootcandy.txt
    echo "\$path = '${RANDOM_NUMBER}';"   >> /root/confcandy/dbrootcandy.txt
    passs=$(grep '$pass' /root/confcandy/dbrootcandy.txt | cut -d"'" -f2)
    userrr=$(grep '$user' /root/confcandy/dbrootcandy.txt | cut -d"'" -f2)
...
# Only runs on a brand-new install (never on resume / Candy's own partial state).
...
# Candy database is created (the fresh-server precheck guarantees no real DB).
repair_mysql() {
    export DEBIAN_FRONTEND=noninteractive
...
# Re-running `candy install` will pick up from the last completed phase.
install_pause() {
    local where="$1"
    echo ""
    echo -e "  ${C_WARN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${CR}"
...
version_section() {
    ...
    _kv "Channel" "${C_DIM}t.me/candypanel${CR}"
    _kv "Group" "${C_DIM}t.me/candypanelgroup${CR}"
}
...
function renew_ssl() {
    ...
    local cfg="/var/www/html/candyprobotconfig/config.php"
    local domain=""
    if [ -f "$cfg" ]; then
    ...
    if ! command -v certbot >/dev/null 2>&1; then
        echo -e "  ${C_BAD}●${CR} ${C_BAD}certbot is not installed. Install Candy first.${CR}"
        sleep 1; show_menu; return 1
    fi
...
function backup_bot() {
    ...
    CONFIG_PATH="/var/www/html/candyprobotconfig/config.php"
    if [ ! -f "$CONFIG_PATH" ]; then
        printf "    ${C_BAD}●${CR} ${C_BAD}Candy is not installed. config.php not found.${CR}\n"
        echo ""
        printf "  ${C_PROMPT}❯${CR} Press Enter to return to the menu... "
        read -r _
        show_menu
    ...
    local backup_file="/root/candy_backup_${backup_date}.sql"
    ...
    if [ -n "$bot_token" ] && [ -n "$admin_id" ]; then
        ...
        send_result=$(curl -s -o /dev/null -w "%{http_code}" \
            ...
            -F "caption=📦 Candy DB Backup (${backup_date})" \
...
function import_bot() {
    ...
    while IFS= read -r f; do
        ...
        i=$((i + 1))
    done < <(find /root -maxdepth 1 -name 'candy_backup_*.sql' -type f 2>/dev/null | sort -r)
...
function show_help_screen() {
    ...
    _sec "Commands"
    _kv "install" "${C_DIM}Install Candy${CR}"
    _kv "update" "${C_DIM}Update Candy (choose channel / version)${CR}"
    _kv "remove" "${C_DIM}Remove Candy and its services${CR}"
    ...
    _sec "Examples"
    printf "    ${C_KEY}candy install --channel auto${CR}\n"
    printf "    ${C_KEY}candy install --name myvpnbot --token 123:ABC \\\\${CR}\n"
    printf "    ${C_DIM}            --admin 111 --domain bot.example.com --version 0.1.7${CR}\n"
    printf "    ${C_KEY}candy update --version 0.1.6${CR}\n"
    printf "    ${C_KEY}candy update --channel release${CR}\n"
    printf "    ${C_KEY}candy remove${CR}\n"
    printf "    ${C_KEY}candy backup${CR}\n"
    printf "    ${C_KEY}candy import${CR}\n"
...
function install_bot() {
    BOT_DIR="/var/www/html/candyprobotconfig"
    ...
    if [ -f "$CONFIG_FILE_DEFAULT" ] && ! has_resumable_state; then
        ...
        printf "    ${C_BAD}●${CR} ${C_BAD}Candy is already installed on this server.${CR}\n"
    ...
    if ! phase_done DEPS; then
        ...
        echo 'phpmyadmin phpmyadmin/app-password-confirm password candyhipass' | sudo debconf-set-selections
        echo 'phpmyadmin phpmyadmin/mysql/admin-pass password candyhipass' | sudo debconf-set-selections
        echo 'phpmyadmin phpmyadmin/mysql/app-pass password candyhipass' | sudo debconf-set-selections
    ...
    if ! phase_done FILES; then
        ...
        TEMP_DIR="/tmp/candyprobot"
        rm -rf "$TEMP_DIR"; mkdir -p "$TEMP_DIR"
        run_step "Downloading Candy (${SRC_LABEL_RESUME})" "wget -O '$TEMP_DIR/bot.zip' '$ZIP_URL'" \
    ...
    if ! phase_done DBROOT; then
        if [ ! -f "/root/confcandy/dbrootcandy.txt" ] || ! grep -q '\$pass' /root/confcandy/dbrootcandy.txt 2>/dev/null; then
    ...
    PATHS=$(cat /root/confcandy/dbrootcandy.txt | grep '$path' | cut -d"'" -f2)
...
EOF
    ...
    ROOT_PASSWORD=$(cat /root/confcandy/dbrootcandy.txt | grep '$pass' | cut -d"'" -f2)
    ROOT_USER="root"
    ...
    if ! phase_done CONFIG; then
        wait
        sleep 1
        file_path="/var/www/html/candyprobotconfig/config.php"
        if [ -f "$file_path" ]; then
            rm "$file_path" || {
        ...
        cat <<EOF > /var/www/html/candyprobotconfig/config.php
<?php
...
?>
EOF
        sudo chown www-data:www-data /var/www/html/candyprobotconfig/config.php 2>/dev/null
        mark_phase CONFIG
    else
        secrettoken="$(state_get SECRET)"
    ...
    if ! phase_done WEBHOOK; then
        ...
        MESSAGE="✅ The Candy bot is installed! for start the bot send /start command."
    ...
    printf "    ${C_OK}●${CR} ${C_OK}Candy is installed and the webhook is set.${CR}\n"
    ...
    _sec "Manage"
    _kv "Command" "${C_DIM}run ${CR}${C_KEY}candy${CR}${C_DIM} anytime to open this panel${CR}"
    echo ""
    _rule
    echo ""

    chmod +x /root/install.sh
    ln -sf /root/install.sh /usr/local/bin/candy
    self_update_script
}
function update_bot() {
    clear
    ...
    if [ ! -d "$BOT_DIR" ]; then
        _sec "Update"
        printf "    ${C_BAD}●${CR} ${C_BAD}Candy is not installed. Install it first.${CR}\n"
        sleep 2
        show_menu
        return 1
    fi
    ...
    print_header "Updating Candy Bot"
    ...
    TEMP_DIR="/tmp/candyprobot_update"
    rm -rf "$TEMP_DIR"; mkdir -p "$TEMP_DIR"
    ...
    TEMP_CONFIG="/root/candypro_config_backup.php"
    if [ -f "$CONFIG_PATH" ]; then
...
EOF
    ...
    rm -rf "$TEMP_DIR"
    echo -e "\n\e[92mCandy Bot updated to latest version successfully!\033[0m"
    if [ -f "/root/install.sh" ]; then
        sudo chmod +x /root/install.sh
        sudo ln -sf /root/install.sh /usr/local/bin/candy
        echo -e "\e[92mEnsured /root/install.sh is executable and 'candy' command is linked.\033[0m"
    else
        echo -e "\e[91mError: /root/install.sh not found after update attempt.\033[0m"
    fi
}
function remove_bot() {
    echo -e "\e[33mStarting Candy Bot removal process...\033[0m"
    LOG_FILE="/var/log/remove_bot.log"
    echo "Log file: $LOG_FILE" > "$LOG_FILE"
    BOT_DIR="/var/www/html/candyprobotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\e[31m[ERROR]\033[0m Candy Bot is not installed (/var/www/html/candyprobotconfig not found)." | tee -a "$LOG_FILE"
        echo -e "\e[33mNothing to remove. Exiting...\033[0m" | tee -a "$LOG_FILE"
        sleep 2
        exit 1
    fi
    read -p "Are you sure you want to remove Candy Bot and its dependencies? (y/n): " choice
    if [[ ! "$choice" =~ ^[Yy]$ ]]; then
        echo "Aborting..." | tee -a "$LOG_FILE"
        exit 0
    fi
    echo "Removing Candy Bot..." | tee -a "$LOG_FILE"
    CONFIG_PATH="/var/www/html/candyprobotconfig/config.php"
    if [ -f "$CONFIG_PATH" ]; then
    ...
    # Clear Candy install state so a fresh install is allowed afterwards
    sudo rm -rf /root/confcandy
    echo -e "\e[92mCandy Bot, MySQL, and their dependencies have been completely removed.\033[0m" | tee -a "$LOG_FILE"
}
...
function migrate_to_pro() {
    ...
    OLD_BOT_DIR="/var/www/html/candybotconfig"
    ...
    BACKUP_FILE="/root/candybot_backup.sql"
    if [ ! -f "$BACKUP_FILE" ]; then
        echo -e "\033[31m[ERROR] Backup file not found at $BACKUP_FILE\033[0m"
        echo -e "\033[33mPlease run the 'candy' command (Free Version Script) and use option 4 to create a backup.\033[0m"
        exit 1
    else
    ...
    echo -e "\033[33mOnly the main bot (candybotconfig) will be migrated.\033[0m"
    echo -e "\033[33mExisting Additional Bots in /var/www/html/ might stop working.\033[0m"
    echo -e "\033[36mFound directories:\033[0m"
    ls -d /var/www/html/*/ 2>/dev/null | grep -v "candybotconfig"
    echo ""
    ...
    ROOT_CRED_FILE="/root/confcandy/dbrootcandy.txt"
    ROOT_PASS=""
    ROOT_USER="root"
    ...
    OLD_CONFIG="/var/www/html/candybotconfig/config.php"
    ...
    NEW_BOT_DIR="/var/www/html/candyprobotconfig"
    rm -rf "$OLD_BOT_DIR"
    mkdir -p "$NEW_BOT_DIR"
    ZIP_URL="https://github.com/ednoct/candybot/archive/refs/heads/main.zip"
    TEMP_DIR="/tmp/candybot_mig"
    mkdir -p "$TEMP_DIR"
    run_step "Downloading Candy source" "wget -q -O '$TEMP_DIR/bot.zip' '$ZIP_URL'" \
        || { show_step_error; echo -e "\033[31mError: Failed to download Candy source.\033[0m"; exit 1; }
...
EOF
    ...
    echo -e "\033[33mUse command 'candy' to manage the bot from now on.\033[0m"
    echo ""
}
...
print_usage() {
  ...
  Candy - management script

  Usage:
    candy [command] [options]

  Commands:
    install            Install Candy
    update             Update Candy
    remove             Remove Candy
  ...
  Examples:
    candy install --channel auto
    candy install --name myvpnbot --token 123:ABC --admin 111 --domain bot.example.com --version 0.1.7
    candy update --channel release
    candy update --version 0.1.6
